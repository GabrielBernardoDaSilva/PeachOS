...
#define PEACHOS_FAT16_UNSED             0x00
...
struct fat_private
{
    ...
    //used to stream teh file allocation table
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t director_start_sector)
{
    struct fat_directory_item item;
    ...
    int res = 0;
    int i = 0;
    int directory_start_pos = director_start_sector * disk->sector_size;
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    int root_directory_sector_pos = (primary_header->fat_copies * primary_header->sectors_per_fat) + primary_header->reserved_sectors;
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_directory_sector_pos)) != PEACHOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
out:
    if (stream)
    {
        diskstreamer_closer(stream);
    }
    if (res < 0)
    {
        kfree(fat_private);
        disk->filesystem = 0x00;
    }
    return res;
}
