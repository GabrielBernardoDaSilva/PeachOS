...
struct path_root* pathparser_parse(const char* path, const char* current_directory_path)
{
    ...
    struct path_part* first_part = pathparser_parser_path_part(NULL, &tmp_path);
    if (!first_part)
        goto out;

    path_root->first = first_part;
    struct path_part* part = pathparser_parser_path_part(first_part, &tmp_path);
    while(part)
        part= pathparser_parser_path_part(part, &tmp_path);
out:
    return path_root;
}